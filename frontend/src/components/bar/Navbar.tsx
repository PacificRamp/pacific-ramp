...
const items = [
  { label: "HOME", link: "/", icon: <Home className="h-[22px] w-[22px]" /> },
  {
    label: "SWAP",
    link: "/swap",
    icon: <ArrowRightLeft className="h-[22px] w-[22px]" />,
  },
  {
    label: "WITHDRAWALS",
    link: "/withdrawals",
    icon: <CircleArrowOutDownLeft className="h-[22px] w-[22px]" />,
  },
  {
    label: "OFFRAMP",
    link: "/offramp",
    icon: <CirclePlus className="h-[22px] w-[22px]" />,
  },
  {
    label: "ONRAMP",
    link: "/onramp",
    icon: <CircleArrowOutUpLeft className="h-[22px] w-[22px]" />,
  },
  {
    label: "PROOF",
    link: "/proof",
    icon: <CircleCheckBig className="h-[22px] w-[22px]" />,
  },
  {
    label: "LIQUIDITY",
    link: "/liquidity",
        ...
        <div className="flex flex-row items-center gap-2">
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <Button
                className="px-3 py-1.5 w-[50px] h-[40px] shadow whitespace-nowrap transition-colors rounded-xl"
                variant={"ghost"}
              >
            ...
            <SheetContent side="right" className="w-[300px]">
              <div className="mt-8 flex flex-col gap-4">
                {items.map((item, index) => (
                  ...
                  >
                    <Label
                      className={`block w-auto p-2 text-sm font-semibold cursor-pointer ${
                        pathname === item.link
        ...
        <div className="flex flex-row justify-start items-center gap-10">
          ...
          <div className="flex flex-row justify-center gap-x-6">
            {items.map((item, index) => (
              ...
              >
                <Label
                  className={`font-semibold cursor-pointer text-[12px] ${
                    pathname === item.link
                      ? "text-textSecondary"
                      : "text-gray-400"
                  }`}
                ...
                <Label
                  className={`font-bold cursor-pointer text-[10px] flex items-center ${
                    pathname === item.link ? "text-white" : "text-gray-400"
                  }`}
                >
                  {item.label}
                </Label>
