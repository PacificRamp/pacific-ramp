...
import { USDM_DECIMALS } from "@/constants/config";
...
export const toUSDCAmount = (amount: string): bigint => {
  ...
  const paddedFraction = fraction
    .padEnd(USDM_DECIMALS, "0")
    .slice(0, USDM_DECIMALS);
...
export const formatNullableAddress = (
  address: string | null | undefined
): string => {
  return address ? formatAddress(address) : "-";
};

export const formatNullableTimestamp = (
